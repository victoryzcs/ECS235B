...
class PolicyEngine:
    ...
    def grant_direct_permission(self, user_id, object_id, action):
    ...
    def add_object(self, obj:Object):
        '''
        Adds an object to the policy engine.
        If the dataset for the object does not exist, it willl be created.
    
        '''
        self.objects[obj.id] = obj
        if obj.dataset in self.datasets: # Check if the dataset exists
            # Add the object to the dataset if it does not exist
    ...
    def record_access(self, user_id, object_id):
        if user_id not in self.users or object_id not in self.objects:
            raise ValueError("Invalid user or object ID")
        ...
        if dataset_id not in self.user_access_history[user_id]:
            ...
            # Update the user's access history in the User module
